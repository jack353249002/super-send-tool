...
import {
  ...
  TOGGLE_MULTI_TAB,
  // i18n
  APP_LANGUAGE, UPDATE_MENU_TYPE, SET_SENDINFO
...
const app = {
  state: {
    ...
    lang: 'en-US',
    _antLocale: {},
    updateMainMenuParent: 0,
    nowSendInfo: {}
  },
  mutations: {
    [SET_SENDINFO]: (state, sendInfo) => {
      state.nowSendInfo = sendInfo
      storage.set(SET_SENDINFO, state.nowSendInfo)
    },
    [UPDATE_MENU_TYPE]: (state) => {
      state.updateMainMenuParent++
      storage.set(UPDATE_MENU_TYPE, state.updateMainMenuParent)
    },
