<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" @click="showReload">更新smtp服务器</a-button>
...
import {
  delSmtpServerCon, getSmtpServerCon,
  RequestConFactory, sendInfoActionCon, setSmtpServerCon,
  setSuperSendOnline
} from '@/api/super_send'
...
export default {
    ...
    return {
      ...
      loadData: parameter => {
         if (this.selectSendInfo !== undefined && this.selectSendInfo.token !== '' && this.selectSendInfo.id !== 0 && this.selectSendInfo.id !== undefined) {
          ...
          return getSmtpServerCon(con, requestParameters)
            .then(res => {
              if (res == null) {
  ...
  methods: {
    ...
    handleDelete (id) {
      if (this.selectSendInfo !== undefined && this.selectSendInfo.token !== '' && this.selectSendInfo.id !== 0) {
        const con = RequestConFactory(this.selectSendInfo)
        delSmtpServerCon(con, id).then(res => {
          if (res.status === 200) {
            this.$refs.table.refresh()
            this.$message.success(res.message)
          } else {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (values.id > 0) {
            ...
            if (this.selectSendInfo !== undefined && this.selectSendInfo.token !== '' && this.selectSendInfo.id !== 0 && this.selectSendInfo.id !== undefined) {
              const con = RequestConFactory(this.selectSendInfo)
              setSmtpServerCon(con, values).then(res => {
                if (res.status === 200) {
                  this.visible = false
